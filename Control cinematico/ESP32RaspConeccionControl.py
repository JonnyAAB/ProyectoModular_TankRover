...
import socket, json, serial, math
...
def pose(encoder_count1, encoder_count2, tiempo_muestreo):
	...
	print("Pose (x, y, theta) = ", x_robot, ", ", y_robot, ", ", theta_robot)
def CrearServidor():
...
try:
	...
	while True:
		...
		while t<tSimulacion:				#Esto es para parar el ciclo por tiempo
			...
			if(t_muestreo >= 0.5):
				pose(encoder_count, encoder_count2, t_muestreo)
				encoder_count = 0
				encoder_count2 = 0
				t_muestreo = 0
